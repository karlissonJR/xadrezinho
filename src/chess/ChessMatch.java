...
public class ChessMatch {
    ...
    private Piece makeMove(Position source, Position target) {
        ChessPiece movePiece = (ChessPiece) board.removePiece(source);
        
        movePiece.increaseMoveCount();
        ...
        // Movimento ROQUE
        if (movePiece instanceof King && target.getColumn() == source.getColumn() + 2) {
    ...
    private boolean testCheck(Color color) {
        ...
        for (Piece opponentPiece : opponentPieces) {
            boolean[][] moves = opponentPiece.possibleMoves();
    ...
    private boolean testCheckMate(Color color) {
        ...
        List<Piece> currentPlayerPieces = piecesOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == color).collect(Collectors.toList());

        for (Piece currentPlayerPiece : currentPlayerPieces) {
            boolean[][] moves = currentPlayerPiece.possibleMoves();
            
            Position source = ((ChessPiece) currentPlayerPiece).getChessPosition().toPosition();
